...
def crossentropy_with_prior(logits: torch.Tensor, labels: torch.Tensor, prior, tau=0.7):
    '''
    the cross entropy loss with prior information
    '''
    log_prior : torch.Tensor = torch.log(prior + 1e-8)
    loss_fct = CrossEntropyLoss()
...
def crossentropy_with_balance(logits: torch.Tensor, labels: torch.Tensor, samples_per_cls, beta=0.9):
    '''
    the class balance loss with prior information

    logits: b_s, seq_len, no_of_cls
    labels: b_s, seq_len
    '''
    ...
    # weights = weights / torch.sum(weights) * len(samples_per_cls)  # labels.shape[-1] = no_of_classes
...
def focal_loss(logits: torch.Tensor, labels: torch.Tensor, samples_per_cls, gamma=2.0):
    '''
    the class balance loss with prior information

    logits: b_s, no_of_cls
    labels: b_s
    '''
    beta = 0.999
    labels = F.one_hot(labels, logits.shape[-1]) # vocab size
    ...
    loss = modulator * loss_fct(logits.view(-1, logits.size(-1)), labels.view(-1))
